...
struct TrocaProvisoriaView: View {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    AWSectionCard(title: "Buscar reserva") {
                        VStack(alignment: .leading, spacing: 12) {
                            AWTextField(
                                placeholder: "Buscar por nº, cliente, placa ou modelo",
                                text: $buscaReserva,
                            ...
                            if form.reservaAtrelada {
                                HStack(spacing: 8) {
                                    ...
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text("Reserva \(form.numeroReserva) atrelada")
                                            .font(AWTheme.headline(13))
                                            .foregroundStyle(AWTheme.success)
                                        Text("Veículo original preenchido automaticamente.")
                            ...
                            if reservasFiltradas.isEmpty {
                                ...
                                Text("Toque na reserva para preencher o veículo original")
                                    .font(AWTheme.caption(12))
                                    .foregroundStyle(AWTheme.textSecondary)
                                
                                VStack(spacing: 8) {
                                    ForEach(reservasFiltradas.prefix(8)) { reserva in
                                        Button {
    ...
    private func aplicarReserva(_ reserva: ReservaEntrega, silencioso: Bool = false) {
        ...
        if !silencioso {
            alertMessage = "Reserva \(reserva.numeroReserva) aplicada. Veículo original preenchido."
            saved = false
            showAlert = true
        }
    }
